                            ...
                            <table class="table table-hover shopping-cart-wrap">
                                ...
                                @foreach(\Cart::getContent() as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <var class="price">
                                                <input data-id="{{ $item->id }}" data-productQuantity="{{ $item->attributes->stock }}" class="quantity" type="number" value="{{ $item->quantity }}" min="1" max="{{ $item->attributes->stock }}">
                                            </var>
                                        </td>
                                        <td>
                                            <div class="price-wrap">
                                                <var class="price">{{ config('settings.currency_symbol'). $item->price }}</var>
                                                <small class="text-muted each">each</small>
                                            </div>
                                        </td>
                                        <td class="text-right">
        ...
        (function(){
            ...
            Array.from(classname).forEach(function(element) {
                element.addEventListener('change', function() {
                    ...
                    axios.patch(`/cart/item/${id}/update`, {
                        quantity: this.value,
